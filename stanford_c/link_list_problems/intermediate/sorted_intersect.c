...
struct node* sorted_intersect(struct node* plista, struct node* plistb);
int main()
{   
    ...
    merge_sort(&plistb);
    print(plistb);
    intersection = sorted_intersect(plista, plistb);
...
struct node* sorted_intersect(struct node* plista, struct node* plistb)
{
    struct node* pnew_list = NULL;
    struct node** ref_plast = &pnew_list;
    struct node* pnew_node;
    while (plista != NULL && plistb != NULL) {
        if (plista->data == plistb->data) {
            prepend(ref_plast, plista->data);
            ref_plast = &((*ref_plast)->next);
            plista = plista->next;
            plistb = plistb->next;
        } else if (plista->data < plistb->data) {
            plista = plista->next;
        } else {
            plistb = plistb->next;
        }
